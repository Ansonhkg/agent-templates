...
## Hosted instruction commands

The gallery now renders per-template curl commands using its current origin and Vite base path. The predev/prebuild generator copies only the three reviewed install guides into public/install; Vite includes them in dist for static hosting. After building, `npm run test:install-guides` checked the displayed commands with real curl requests against both the running local demo and a separate production preview, comparing all three responses exactly with canonical Markdown. Run the local demo on port 4328 first. All 11 clipboard checks also passed. This verifies the deployable files; no public host/domain was provisioned by this change.

## Simplified gallery and Demo/Source views

Removed header navigation/account controls, gallery metadata/footer, and duplicate card links. Both templates now have keyboard-accessible Demo/Source tabs with a flip transition, reduced-motion support, reloadable URLs, and preserved drafts. The composer has a 98px container and one background in idle/focus states. Browser checks verified these behaviors, twelve copyable code blocks, mobile overflow, and all guide curl endpoints on development and production previews. Automated accessibility scans reported zero violations across the gallery, both demo variants, both source views, and the account dialog after animations settled. Build passed. The unit suite had one filesystem fixture timing failure on the initial run; all eighteen checks passed on rerun, without changing runtime behavior. No image generation rerun was needed for these presentation changes.
